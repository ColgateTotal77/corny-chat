...
void on_add_group_button_clicked(GtkWidget *widget, gpointer data) {
    ...
    if (first_monitor) {
        ...
        window_width = geometry.width * 0.2;
    }
...
void on_settings_group_button_clicked(GtkWidget *button, gpointer user_data) {
    ...
    if (first_monitor) {
        ...
        // Calculate window size based on FIRST monitor geometry
        window_width = geometry.width * 0.4;
        window_height = geometry.height * 0.5;
    }
    ...
    gtk_widget_set_hexpand(entry_container_2, TRUE);
    ...
    gtk_widget_set_hexpand(name_entry_2, TRUE);
    ...
    gtk_grid_attach(GTK_GRID(grid), small_button, 0, 1, 1, 1);
    gtk_widget_set_hexpand(small_button, TRUE);
    gtk_widget_set_vexpand(small_button, FALSE);
    gtk_widget_set_size_request(small_button, -1, 60);
...
static void on_activate(GtkApplication *app, gpointer user_data) {
    ...
    if (first_monitor) {
        ...
        // Calculate window size based on FIRST monitor geometry
        window_width = geometry.width * 0.8;
    ...
    gtk_widget_set_hexpand(sidebar_background, FALSE);
    gtk_widget_set_vexpand(sidebar_background, TRUE);
    ...
    gtk_widget_set_vexpand(sidebar_users, TRUE); 
    ...
    gtk_widget_set_visible(sidebar_scroll_groups, FALSE);
    gtk_widget_set_vexpand(sidebar_groups, TRUE);
    ...
    GtkWidget *left_button = gtk_button_new();
    gtk_widget_add_css_class(left_button, "left-button");
    gtk_widget_set_size_request(left_button, 30, 30);
    gtk_box_append(GTK_BOX(sidebar_container), left_button);

    GtkWidget *back_icon = gtk_image_new_from_file("src/chat_visual/images/back.svg");
    gtk_button_set_child(GTK_BUTTON(left_button), back_icon);
    gtk_widget_set_size_request(back_icon, 24, 24);

    // Center label
    GtkWidget *center_label = gtk_label_new("Chats");
    gtk_widget_add_css_class(center_label, "center-label");
    gtk_widget_set_size_request(center_label, 100, 50);
    gtk_label_set_ellipsize(GTK_LABEL(center_label), PANGO_ELLIPSIZE_END);
    ...
    GtkWidget *right_button = gtk_button_new();
    gtk_widget_add_css_class(right_button, "right-button");
    gtk_widget_set_size_request(right_button, 30, 30);
    gtk_box_append(GTK_BOX(sidebar_container), right_button);

    GtkWidget *next_icon = gtk_image_new_from_file("src/chat_visual/images/forward_arrow.svg");
    gtk_button_set_child(GTK_BUTTON(right_button), next_icon);
    gtk_widget_set_size_request(next_icon, 24, 24);
    
    gtk_box_set_homogeneous(GTK_BOX(sidebar_container), TRUE);
    ...
    gtk_widget_set_hexpand(settings_group_button, FALSE);
    gtk_widget_set_halign(settings_group_button, GTK_ALIGN_END);
