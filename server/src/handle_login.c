...
static void handle_no_such_user(call_data_t *call_data, int user_id, char *login) {
    char buff_out[BUF_SIZE];
    client_t *client_data = call_data->client_data;
    ht_t *clients = call_data->general_data->clients;
    
    user_t *user_data = init_user_data(user_id, login, login, true);
	client_data->user_data = user_data;
	client_data->ssl = call_data->ssl;
    ht_set(clients, user_data->user_id, (void*)client_data);
    ht_str_set(call_data->general_data->login_to_id, login, user_id);

    create_and_send_succesfull_login_data(call_data, user_id, login);
...
int handle_login(char *str_json_login_password, call_data_t *call_data) {
	...
	is_valid_login = find_or_create_user(call_data, json_login_password, &user_id);
	...
	if (is_valid_login == INVALID_INPUT) {
        ...
        printf("No such user\n");
		handle_no_such_user(call_data, user_id, login->valuestring);
	}
