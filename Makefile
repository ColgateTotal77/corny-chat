# Compiler and flags
CC = clang
CFLAGS = -std=c11 -Wall -Wextra -Werror -Wpedantic -D_POSIX_C_SOURCE $(shell pkg-config --cflags gtk4)
LDFLAGS = -lssl -lcrypto $(shell pkg-config --libs gtk4)
...
$(CLIENT_OBJ)/start_login.o: $(LOGIN_VISUAL)/start_login.c
	@mkdir -p $(CLIENT_OBJ)
	$(CC) $(CFLAGS) -o $@ -c $< -I $(HEADERFILES)
